defmodule Entitiex.ConditionsTest do
  ...
  test "compile" do
    ...
    assert Conditions.compile(Test, [expose_nil: false]) == [&Conditions.not_nil?/2]
    ...
    assert Conditions.compile(Test, [if: :should_expose?, expose_nil: false]) == [&Conditions.not_nil?/2, &Test.should_expose?/2]
    assert Conditions.compile(Test, [if: :should_expose3?]) == [&Test.should_expose3?/3]
    assert Conditions.compile(Test, [if: :should_expose3?, expose_nil: false]) == [&Conditions.not_nil?/2, &Test.should_expose3?/3]
  end

  test "run" do
    assert Conditions.run([&Test.should_expose?/2], %{}, "expose me")
    refute Conditions.run([&Test.should_expose?/2], %{}, "do not expose me")
    refute Conditions.run([&Conditions.not_nil?/2, &Test.should_always_expose?/2], %{}, nil)
    assert Conditions.run([&Conditions.not_nil?/2, &Test.should_always_expose?/2], %{}, "expose me")
